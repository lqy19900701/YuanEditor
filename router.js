 ...
 */
router.get('/', function (req, res, next) {
    db.query('select * from nameedit', function (err, rows) {
        console.log(rows);
        if (err) {
            res.render('nameedit', {title: '目录', datas: []});  // this renders "views/persons.html"
        } else {

            res.render('nameedit', {title: '目录', datas: rows});
        }
    })
});
/**
